...
char Ascii(const int brightness) {
  return kAsciiMap[brightness];
}
...
struct Image {
  ...
  const int n,   m;
  Image(const std::vector<std::vector<Pixel>>& pixels_) : pixels(pixels_), n(pixels.size()), m(pixels[0].size()) {}

  static Image BuildImage(const std::string& image_filepath) {
    std::ifstream fin(image_filepath);
    std::string header;
    ...
    for (auto [i, line] : pixels | std::views::enumerate) {
      line.reserve(m);
      for (int j = 0; j < m; ++j) {
        const uint8_t r = raw_pixels[i*m + j], g = raw_pixels[i*m + j + 1], b = raw_pixels[i*m + j + 2];
        line.emplace_back(r, g, b);
      }
    }
    std::println("Built {} x {} image | pixels[100][:4]: {}", n, m, pixels[100] | std::views::take(5));
    return Image(pixels);
  }
  ...
  void PrintAscii(const int letter_size) const {
    ...
    for (int i = 0; i + letter_size - 1 < n; i += letter_size) {
      for (int j = 0; j + letter_size - 1 < m; j += letter_size) {
        int r = 0, g = 0, b = 0;
